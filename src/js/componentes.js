...
const anchorFiltro = document.querySelectorAll('.filtro'); // Cambiado a querySelectorAll
...
divTodoList.addEventListener('click', (evento) => {
    ...
    const todoElemento = evento.target.closest('li'); // Usar closest para encontrar el li
...
ulFiltros.addEventListener('click', (evento) => {
    const filtro = evento.target.textContent; // Cambiado a textContent
    if (!filtro) { return; }
    ...
    for (const elemento of divTodoList.children) {
        ...
        switch (filtro) {
            ...
            case 'Completados':
                if (!completado) {
                    elemento.classList.add('hidden'); // Corregido a 'hidden'
                }
                break;
        }
    }
